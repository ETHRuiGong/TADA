...
from model.deeplabv2_outputfeat import Res_Deeplab
...
from data import get_loader_coarsetofine_fewshot_numsample_classspecific, get_data_path
...
from evaluateTACS import evaluate

import time

from loss_contrastive_uncertaintyaware import PixelContrastLoss_UncertaintyAware
...
def get_arguments():
    ...
    parser.add_argument("--numsamples", type=int, default=None, help="number of few-shot samples")
...
def strongTransform_onlymix(parameters, data=None, target=None):
    assert ((data is not None) or (target is not None))
    data, target = transformsgpu.oneMix(mask=parameters["Mix"], data=data, target=target)
...
def _resume_checkpoint(resume_path, model, optimizer, ema_model):
    ...
    if train_unlabeled:
        print("resume ema model from the model checkpoint !!!!!!")
        if len(gpus) > 1:
...
def main():
    ...
    seg_contraloss = PixelContrastLoss_UncertaintyAware(temperature=0.07, base_temperature=0.07)
    ...
    if dataset == 'cityscapes':
        data_loader = get_loader_coarsetofine_fewshot_numsample_classspecific('cityscapes')
        ...
        data_loader = get_loader_coarsetofine_fewshot_numsample_classspecific('cityscapes_fewshot')
        data_path = get_data_path('cityscapes_fewshot')
        if random_crop:
            data_aug = Compose([RandomCrop_city(input_size)])
        else:
            data_aug = None
        ...
        train_dataset_fewshot = data_loader(data_path, is_transform=True, augmentations=data_aug, img_size=input_size,
                                            img_mean=IMG_MEAN, num_fewshotsamples=args.numsamples)
    ...
    if labeled_samples is None:
        ...
        trainloader_remain_fewshot = data.DataLoader(train_dataset_fewshot, batch_size=batch_size, shuffle=True,
                                                     num_workers=num_workers, pin_memory=True)
        trainloader_remain_fewshot_iter = iter(trainloader_remain_fewshot)
    ...
    if True:
        data_loader = get_loader_coarsetofine_fewshot_numsample_classspecific('gta')
    ...
    for i_iter in range(start_iteration, num_iterations):
        ...
        ###### There is some possibility that the coarse class will not be assigned to the assigned fine class###

        prob_select = torch.rand(labels.shape).cuda()
        labels[(prob_select >= 0.5) & (labels == 8)] = 9
        labels[(prob_select >= 0.5) & (labels == 17)] = 18
        labels[(prob_select >= 0.5) & (labels == 11)] = 12

        ################ Add Relabel Part to the Source Domain #####################
        pred_relabel = interp(ema_model(images)[1])
        pred_relabel = torch.softmax(pred_relabel.detach(), dim=1)
        prob_select, pred_relabel = torch.max(pred_relabel.detach(), dim=1)
        ##### Coarse-to-Fine Setting #############
        # VEG ---> Vegetation (8) and Terrain (9)
        # CYCLE ---> Bicycle (18) and Motorcycle (17)
        # People ---> Person (11) and Rider (12)

        labels[(pred_relabel == 8) & (labels == 9) & (prob_select >= 0.0)] = 8
        labels[(pred_relabel == 9) & (labels == 8) & (prob_select >= 0.0)] = 9
        labels[(pred_relabel == 18) & (labels == 17) & (prob_select >= 0.0)] = 18
        labels[(pred_relabel == 17) & (labels == 18) & (prob_select >= 0.0)] = 17
        labels[(pred_relabel == 11) & (labels == 12) & (prob_select >= 0.0)] = 11
        labels[(pred_relabel == 12) & (labels == 11) & (prob_select >= 0.0)] = 12

        ################ For the label after relabeling, there is some possibility to be randomized to prevent overfitting ################
        prob_select = torch.rand(labels.shape).cuda()
        labels_orig = labels.clone()
        labels[(prob_select >= 0.55) & (labels_orig == 8)] = 9
        labels[(prob_select >= 0.55) & (labels_orig == 9)] = 8
        labels[(prob_select >= 0.55) & (labels_orig == 17)] = 18
        labels[(prob_select >= 0.55) & (labels_orig == 18)] = 17
        labels[(prob_select >= 0.55) & (labels_orig == 11)] = 12
        labels[(prob_select >= 0.55) & (labels_orig == 12)] = 11




        pred = interp(model(images)[1])
        ...
        if train_unlabeled:
            ...
            try:
                batch_remain_fewshot = next(trainloader_remain_fewshot_iter)
                if batch_remain_fewshot[0].shape[0] != batch_size:
                    batch_remain_fewshot = next(trainloader_remain_fewshot_iter)
            except:
                trainloader_remain_fewshot_iter = iter(trainloader_remain_fewshot)
                batch_remain_fewshot = next(trainloader_remain_fewshot_iter)

            images_remain_fewshot, _, labels_fewshot, _, _, _ = batch_remain_fewshot
            images_remain_fewshot = images_remain_fewshot.cuda()
            labels_fewshot = labels_fewshot.cuda().long().detach()

            inputs_u_w, _ = weakTransform(weak_parameters, data=images_remain)
            # inputs_u_w = inputs_u_w.clone()
            logits_u_w = interp(ema_model(inputs_u_w)[1])
            logits_u_w, _ = weakTransform(getWeakInverseTransformParameters(weak_parameters), data=logits_u_w.detach())

            pseudo_label = torch.softmax(logits_u_w.detach(), dim=1)
            max_probs, targets_u_w = torch.max(pseudo_label, dim=1)

            # ###################### Unify the pseudolabel and the annotated class on the target domain #################################
            # Generate the inputs by further mixing the few-shot cityscapes and the generated images ############################
            if mix_mask == "class":
                for image_i in range(batch_size):
                    classes = torch.unique(labels_fewshot[image_i])
                    ...
                    if image_i == 0:
                        MixMask0 = transformmasks.generate_class_mask(labels_fewshot[image_i], classes).unsqueeze(
                            0).cuda()
                    else:
                        MixMask1 = transformmasks.generate_class_mask(labels_fewshot[image_i], classes).unsqueeze(
                            0).cuda()
            ...
            inputs_u_s0, _ = strongTransform_onlymix(strong_parameters, data=torch.cat(
                (images_remain_fewshot[0].unsqueeze(0), images_remain[0].unsqueeze(0))))
            strong_parameters["Mix"] = MixMask1
            inputs_u_s1, _ = strongTransform_onlymix(strong_parameters, data=torch.cat(
                (images_remain_fewshot[1].unsqueeze(0), images_remain[1].unsqueeze(0))))
            inputs_u_s = torch.cat((inputs_u_s0, inputs_u_s1))

            strong_parameters["Mix"] = MixMask0
            _, targets_u0 = strongTransform_onlymix(strong_parameters, target=torch.cat(
                (labels_fewshot[0].unsqueeze(0), targets_u_w[0].unsqueeze(0))))
            strong_parameters["Mix"] = MixMask1
            _, targets_u1 = strongTransform_onlymix(strong_parameters, target=torch.cat(
                (labels_fewshot[1].unsqueeze(0), targets_u_w[1].unsqueeze(0))))
            ...
            strong_parameters["Mix"] = MixMask0
            _, pixelWiseWeight0 = strongTransform(strong_parameters, target=torch.cat(
                (onesWeights[0].unsqueeze(0), pixelWiseWeight[0].unsqueeze(0))))
            strong_parameters["Mix"] = MixMask1
            _, pixelWiseWeight1 = strongTransform(strong_parameters, target=torch.cat(
                (onesWeights[1].unsqueeze(0), pixelWiseWeight[1].unsqueeze(0))))
            ...
            inputs_u_s0, _ = strongTransform(strong_parameters,
                                             data=torch.cat((images[0].unsqueeze(0), inputs_u_s[0].unsqueeze(0))))
            strong_parameters["Mix"] = MixMask1
            inputs_u_s1, _ = strongTransform(strong_parameters,
                                             data=torch.cat((images[1].unsqueeze(0), inputs_u_s[1].unsqueeze(0))))
            inputs_u_s = torch.cat((inputs_u_s0, inputs_u_s1))

            strong_parameters["Mix"] = MixMask0
            _, targets_u0 = strongTransform(strong_parameters,
                                            target=torch.cat((labels[0].unsqueeze(0), targets_u[0].unsqueeze(0))))
            strong_parameters["Mix"] = MixMask1
            _, targets_u1 = strongTransform(strong_parameters,
                                            target=torch.cat((labels[1].unsqueeze(0), targets_u[1].unsqueeze(0))))
            targets_u = torch.cat((targets_u0, targets_u1)).long()
            ...
            ################# Input the mixed images to the model #######################
            logits_u_s_feat, logits_u_s = model(inputs_u_s)
            ########## Calculate Contrastive Loss ########
            L_contra = seg_contraloss(logits_u_s_feat, targets_u,
                                      torch.max(logits_u_s, dim=1)[1],
                                      torch.max(torch.softmax(logits_u_s.detach(), dim=1), dim=1)[
                                          0])  # feats, labels=None, predict=None
        ...
        if len(gpus) > 1:
            ...
            if train_unlabeled:
                ...
                loss_contra_value = L_contra.mean().item()
        else:
            loss_l_value += L_l.item()
            if train_unlabeled:
                loss_u_value += L_u.item()
                loss_contra_value = L_contra.item()
        ...
        print('iter = {0:6d}/{1:6d}, loss_l = {2:.3f}, loss_u = {3:.3f}, loss_contra = {4:.3f}'.format(i_iter,
                                                                                                       num_iterations,
                                                                                                       loss_l_value,
                                                                                                       loss_u_value,
                                                                                                       loss_contra_value))
        ...
        if config['utils']['tensorboard']:
            ...
            if train_unlabeled:
                ...
                accumulated_loss_contra.append(loss_contra_value)
            ...
            if i_iter % log_per_iter == 0 and i_iter != 0:
                ...
                if train_unlabeled:
                    ...
                    tensorboard_writer.add_scalar('Training/Contrastive loss', np.mean(accumulated_loss_contra), i_iter)
                    accumulated_loss_contra = []

        if i_iter % val_per_iter == 0 and i_iter != 0:
            model.eval()
            if dataset == 'cityscapes':
                mIoU, eval_loss = evaluate(model, dataset, ignore_label=255, input_size=(512, 1024),
                                           save_dir=checkpoint_dir)  # 250
    ...
    model.eval()
    if dataset == 'cityscapes':
        mIoU, val_loss = evaluate(model, dataset, ignore_label=255, input_size=(512, 1024),
                                  save_dir=checkpoint_dir)  # 250
    model.train()
    if mIoU > best_mIoU and save_best_model:
        best_mIoU = mIoU
...
if __name__ == '__main__':
    ...
    if args.resume:
        ...
        checkpoint_dir = os.path.join(config['utils']['checkpoint_dir'], args.name + '_resume')
    else:
        checkpoint_dir = os.path.join(config['utils']['checkpoint_dir'], args.name + '_resume')
