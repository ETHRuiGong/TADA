...
from data import get_loader_annotationadaptation_fewshot_numsample_classspecific, get_data_path
...
def main():
    ...
    if dataset == 'cityscapes':
        data_loader = get_loader_annotationadaptation_fewshot_numsample_classspecific('cityscapes')
        ...
        data_loader = get_loader_annotationadaptation_fewshot_numsample_classspecific('cityscapes_fewshot')
    ...
    if labeled_samples is None:
        ...
        trainloader_remain_fewshot = data.DataLoader(train_dataset_fewshot, batch_size=batch_size, shuffle=True, num_workers=num_workers, pin_memory=True)
    ...
    if True:
        data_loader = get_loader_annotationadaptation_fewshot_numsample_classspecific('synthia')
        ...
        train_dataset = data_loader(data_path, list_path='./data/synthia_list/train.txt', augmentations=data_aug,
    ...
    for i_iter in range(start_iteration, num_iterations):
        ...
        _, pred_relabel = torch.max(pred_relabel.detach(), dim=1)
        # New classes wall fence pole, terrain, truck, train ##########
        # New classes index 3, 4, 5, 9, 14, 16 ######################
        labels[(pred_relabel == 3) & (labels == 255)] = 3
        labels[(pred_relabel == 4) & (labels == 255)] = 4
        labels[(pred_relabel == 5) & (labels == 255)] = 5
        labels[(pred_relabel == 9) & (labels == 255)] = 9
        labels[(pred_relabel == 14) & (labels == 255)] = 14
        labels[(pred_relabel == 16) & (labels == 255)] = 16
        ...
        if train_unlabeled:
            ...
            if mix_mask == "class":
                for image_i in range(batch_size):
                    ...
                    # print("classes before remove.....", classes)
            ...
            inputs_u_s0, _ = strongTransform(strong_parameters, data = torch.cat((images[0].unsqueeze(0),inputs_u_s[0].unsqueeze(0))))
            strong_parameters["Mix"] = MixMask1
            inputs_u_s1, _ = strongTransform(strong_parameters, data = torch.cat((images[1].unsqueeze(0),inputs_u_s[1].unsqueeze(0))))
            inputs_u_s = torch.cat((inputs_u_s0,inputs_u_s1))

            strong_parameters["Mix"] = MixMask0
            _, targets_u0 = strongTransform(strong_parameters, target = torch.cat((labels[0].unsqueeze(0),targets_u[0].unsqueeze(0))))
            strong_parameters["Mix"] = MixMask1
            _, targets_u1 = strongTransform(strong_parameters, target = torch.cat((labels[1].unsqueeze(0),targets_u[1].unsqueeze(0))))
            targets_u = torch.cat((targets_u0,targets_u1)).long()
            ...
            L_contra = seg_contraloss(logits_u_s_feat, targets_u,
                                      ...
                                      torch.max(torch.softmax(logits_u_s.detach(), dim=1), dim=1)[0])  # feats, labels=None, predict=None
        ...
        print('iter = {0:6d}/{1:6d}, loss_l = {2:.3f}, loss_u = {3:.3f}, loss_contra = {4:.3f}'.format(i_iter, num_iterations, loss_l_value, loss_u_value, loss_contra_value))
        ...
        if i_iter % val_per_iter == 0 and i_iter != 0:
            model.eval()
            if dataset == 'cityscapes':
                mIoU, eval_loss = evaluate(model, dataset, ignore_label=255, input_size=(512,1024), save_dir=checkpoint_dir) # 250
    ...
    model.eval()
    if dataset == 'cityscapes':
        mIoU, val_loss = evaluate(model, dataset, ignore_label=255, input_size=(512,1024), save_dir=checkpoint_dir) # 250
    model.train()
    if mIoU > best_mIoU and save_best_model:
        best_mIoU = mIoU
        _save_checkpoint(i_iter, model, optimizer, config, ema_model, save_best=True)
