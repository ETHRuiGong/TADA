...
class cityscapesLoader_coarsetofine_fewshot_numsample_classspecific_extremecase(data.Dataset):
    """cityscapesLoader
    ...
    def __init__(
        ...
        if split == 'train':
            ...
            # EXTREME CASES: PERSON, RIDER, CAR, TRUCK, BUS, TRAIN, MOTORCYCLE, BIKE
            with open("./process_data/person_coarsetofine.txt", 'r') as person_file:
                self.person_file = person_file.readlines()
            person_index = random.sample(list(range(len(self.person_file))), num_sample)
            with open("./process_data/rider_coarsetofine.txt", 'r') as rider_file:
                self.rider_file = rider_file.readlines()
            rider_index = random.sample(list(range(len(self.rider_file))), num_sample)
            with open("./process_data/motorcycle_coarsetofine.txt", 'r') as motorcycle_file:
                self.motorcycle_file = motorcycle_file.readlines()
            motorcycle_index = random.sample(list(range(len(self.motorcycle_file))), num_sample)
            with open("./process_data/bicycle_coarsetofine.txt", 'r') as bicycle_file:
                self.bicycle_file = bicycle_file.readlines()
            bicycle_index = random.sample(list(range(len(self.bicycle_file))), num_sample)


            with open("./process_data/car_conflictclass.txt", 'r') as car_file:
                self.car_file = car_file.readlines()
            car_index = random.sample(list(range(len(self.car_file))), num_sample)
            with open("./process_data/truck_conflictclass.txt", 'r') as truck_file:
                self.truck_file = truck_file.readlines()
            truck_index = random.sample(list(range(len(self.truck_file))), num_sample)
            with open("./process_data/bus_conflictclass.txt", 'r') as bus_file:
                self.bus_file = bus_file.readlines()
            bus_index = random.sample(list(range(len(self.bus_file))), num_sample)
            with open("./process_data/trainn_conflictclass.txt", 'r') as trainn_file:
                self.trainn_file = trainn_file.readlines()
            trainn_index = random.sample(list(range(len(self.trainn_file))), num_sample)


            print("Coarse to Fine Extremecase!!!!!!!!!!!!!!!!!!")
            ...
            for car_id in car_index:
                self.files[split].append(os.path.join(self.images_base, self.car_file[car_id].strip()))
                self.car_selected.append(os.path.join(self.images_base, self.car_file[car_id].strip()))
            ...
            for bus_id in bus_index:
                self.files[split].append(os.path.join(self.images_base, self.bus_file[bus_id].strip()))
                self.bus_selected.append(os.path.join(self.images_base, self.bus_file[bus_id].strip()))
        ...
        self.id_to_trainid_annotated = {11: 11, 12: 12, 13:13, 14:14, 15:15, 16:16, 17: 17, 18: 18}
    ...
    def __getitem__(self, index):
        ...
        if img_path in self.car_selected:
            id_classspecific.update(self.car_id)

        if img_path in self.truck_selected:
            id_classspecific.update(self.truck_id)

        if img_path in self.bus_selected:
            id_classspecific.update(self.bus_id)
