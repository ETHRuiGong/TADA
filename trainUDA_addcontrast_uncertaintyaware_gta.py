...
from loss_contrastive_uncertaintyaware_removeNone import PixelContrastLoss_UncertaintyAware
    ...
    for i_iter in range(start_iteration, num_iterations):
        ...
        if train_unlabeled:
            ...
            _, targets_u0 = strongTransform(strong_parameters,
                                            target=torch.cat((labels[0].unsqueeze(0), targets_u_w[0].unsqueeze(0))))
            strong_parameters["Mix"] = MixMask1
            _, targets_u1 = strongTransform(strong_parameters,
                                            target=torch.cat((labels[1].unsqueeze(0), targets_u_w[1].unsqueeze(0))))
            ...
            L_contra = seg_contraloss(logits_u_s_feat, targets_u,
                                      torch.max(logits_u_s, dim=1)[1], torch.max(torch.softmax(logits_u_s.detach(), dim=1), dim=1)[0])  # feats, labels=None, predict=None
...
if __name__ == '__main__':
    ...
    if args.resume:
        ...
        checkpoint_dir = os.path.join(config['utils']['checkpoint_dir'], args.name + start_writeable + '_resume')
    else:
